...
ABatteryCollectorCharacter::ABatteryCollectorCharacter()
{
	...
	//set a base power level for our character
	InitialPower = 2000.f;
	CharacterPower = InitialPower;

	//set the dependence of the speed and the power level
	SpeedFactor = 0.75f;
	BaseSpeed = 10.0f;
...
void ABatteryCollectorCharacter::CollectPickups()
{
	...
	for (int32 iCollected = 0; iCollected < CollectedActors.Num(); iCollected++)
	{
		...
		if (TestPickup && !TestPickup->IsPendingKill() && TestPickup->IsActive())
		{
			...
			if (TestBattery)
			{
				...
				CollectedBatteryPower += TestBattery->GetPower();
			}
			// Dectivate the pickup
	...
	{
		UpdatePower(CollectedBatteryPower);
	}
}
...
void ABatteryCollectorCharacter::UpdatePower(float PowerChange)
{
	...
	//GetCharacterMovement is from the base character class that is ABatterCharacterCollector inherits from 
